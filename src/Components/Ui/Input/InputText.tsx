...
export default function InputText({ register,Err,label,type,show}: Props) {
  ...
  return (
    <div className="mb-2 relative">
        <label htmlFor="email" className="font-bold text-slate-400"> {label} </label>
        <input
            {...register}
            placeholder={`Entrez votre ${label}`}
            type={show ? showPassword :  type  }
            className={`w-full border-b-4 py-2 pl-2 border-orange-500 focus:outline-none focus:border-s-4 rounded-lg ${Err ? "mb-0" : "mb-4"}  `}
            id="email"
        />
